...
lm_svy_mi <- function(formula, imputations) {
  ...
  for(i in 1:imputations$m) {
    ...
    null_model <- svyglm(bp_uncontrolled_140_90~1, design = imputation.svy, family = quasibinomial)
    R2 <- c(R2,1 - (logLik(model)/logLik(null_model)))
...
convertModel <- function(model) {
  tr <- createTexreg(
    ...
    gof.names=c("Mean R2","Mean AIC"),
    gof = c( model$R2,model$aic),
